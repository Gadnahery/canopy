...
function tone(p: number | null) {
  if (p == null) return "text-text-secondary";
  if (p >= 70) return "text-primary";
  if (p >= 40) return "text-primary-600";
  return "text-warning";
}
...
export default function CaptureCard({ c, i, onClick }: { c: Capture; i: number; onClick: () => void }) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between px-3.5 py-2.5 text-xs text-text-secondary">
